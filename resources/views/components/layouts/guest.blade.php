...
<head>
    ...
    <!-- Google tag (gtag.js) -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=G-287LRG60M0"></script>
    <script>
    ...
    <div id="main-content">
        ...
        <header class="flex justify-between items-center px-6 py-4 shadow-sm dark:shadow-gray-800">
            <a href="{{ route('home') }}" wire:navigate>
                <img src="{{ asset('images/nexo.png') }}" alt="Logo"
                ...
                @else
                    ...
                    @if (!request()->routeIs('login'))
                        <a href="{{ route('login') }}" wire:navigate
                            class="px-4 py-2 text-sm rounded-lg bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 transition">Iniciar
                            Sesión</a>
                    @endif
                    ...
                    @if (!request()->routeIs('register'))
                        <a href="{{ route('register') }}" wire:navigate
                            class="px-4 py-2 text-sm rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition">Registrarse</a>
                    @endif
                @endauth
